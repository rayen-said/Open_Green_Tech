...
export function WorkspaceDashboard() {
  ...
  const anomalyWindowPct = useMemo(() => {
    if (telemetryFeed.length === 0) {
      return null;
    }
    const flagged = telemetryFeed.filter((e) => e.anomaly).length;
    return Math.round((flagged / telemetryFeed.length) * 100);
  }, [telemetryFeed]);
      ...
      <section className="workspace">
          ...
          <>
            <div className="metrics-grid">
              ...
              <article>
                ...
                <h3>{anomalyWindowPct != null ? `${anomalyWindowPct}%` : "--"}</h3>
              </article>
            </div>
        ...
        {tab === "recommendations" ? (
          <section className="card recommendation-list">
            ...
            {(() => {
              const warns = recommendations.flatMap((r) =>
                r.detectedIssues.filter((i) => i.startsWith("ai_warning:")).map((i) => i.slice("ai_warning:".length)),
              );
              return warns.length > 0 ? (
                <div className="secondary-warning" style={{ marginBottom: "0.9rem", padding: "0.75rem 0.9rem", borderRadius: 12 }}>
            ...
            {recommendations.map((item) => (
              <article key={item.id}>
                ...
                <div className="recommendation-metadata">
                  ...
                  <small>
                    {t("recommendations.issues")}:{" "}
                    {item.detectedIssues.filter((i) => !i.startsWith("ai_warning:")).length > 0
                      ? item.detectedIssues.filter((i) => !i.startsWith("ai_warning:")).map(issueLabel).join(", ")
                      : "none"}
                  </small>
                  <small>
