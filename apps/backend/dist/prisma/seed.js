...
function buildTelemetry(index, deviceIndex) {
...
async function main() {
    ...
    const passwordHash = await (0, bcryptjs_1.hash)('Admin@12345', 10);
    const userPasswordHash = await (0, bcryptjs_1.hash)('Farmer@12345', 10);
    ...
    const devices = await Promise.all([
        ...
        prisma.device.create({
            data: {
                ...
                soilType: 'clay loam',
                cropType: 'citrus',
                status: client_1.DeviceStatus.MAINTENANCE,
                ownerId: admin.id,
            },
        }),
    ]);
    const telemetrySeeds = devices.flatMap((device, deviceIndex) => Array.from({ length: 40 }, (_, index) => ({
        deviceId: device.id,
    ...
    const latestByDevice = await Promise.all(devices.map((device) => prisma.telemetry.findFirst({
